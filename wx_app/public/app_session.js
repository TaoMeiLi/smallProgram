...
const loginAndGetExt = (succCallback, failCallback)=>{
  wx.p.login().then(loginRes=>{
    console.log('[session.js] login result', loginRes);
    if (!loginRes.code) {
...
const onSessionFail = ()=>{
  ...
  loginAndGetExt(obj=>{
    ...
    console.log('[session.js] initapp request _params', _params);
    
    if (_params.hasOwnProperty('login_state')) {
      delete _params.login_state;
    }
    ...
    init(_app, _params, result=>{ //fetch
      ...
      if(_params && _params.referrerInfo && _params.referrerInfo.extraData) {
        const _extraData = _params.referrerInfo.extraData;
        if(_extraData.unionid && _extraData.userInfo) {
          extendMyParams({unionid: _extraData.unionid});
          console.log('[session.js] 从会员价进入点餐 userInfo', JSON.parse(_extraData.userInfo));
          onUserinfoGot({userInfo: JSON.parse(_extraData.userInfo)});
          return;
        }
      }
...
const finish = (isFromLocal=false)=>{
  ...
  console.log('[session.js] reLaunch after login()', _params.path, getCurrentPages(), _app.globalData, 'end login');
  ...
  if (_params.query && Object.keys(_params.query).length > 0) {
  ...
  if (url.indexOf('/scan/scan') >= 0) {
    url = `${url}?isScan=true`;
  }
  console.log('[session.js] url reLaunch url', url);
